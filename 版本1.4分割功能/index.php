...
if (isset($_POST['claim_train'])) {
    $u_check = $conn->query("SELECT last_train_time FROM users WHERE id = $user_id")->fetch_assoc();
    $last_train = $u_check['last_train_time'] ? new DateTime($u_check['last_train_time']) : null;
    if ($last_train) {
        $now = new DateTime();
        $seconds_diff = $now->getTimestamp() - $last_train->getTimestamp();
        if ($seconds_diff >= $cooldown_seconds) {
            $conn->query("UPDATE users SET last_train_time = NULL, exp = exp + 50, stat_points = stat_points + 1 WHERE id = $user_id");
            $msg .= "<span style='color:#ffeb3b;'>🎁 訓練完成！獲得 50 EXP 與 <b>1 點自由屬性點</b>！</span><br>";
        }
    }
}
...
while ($user['exp'] >= $user['level'] * 100) {
    ...
    $new_exp = $user['exp'] - $exp_needed;
    $user['level']++; $user['exp'] = $new_exp;
    $user['dmg'] += 3; $user['max_hp'] += 10; $user['def'] += 1; $user['hp'] = $user['max_hp'];
    $conn->query("UPDATE users SET level = {$user['level']}, exp = {$user['exp']}, dmg = {$user['dmg']}, max_hp = {$user['max_hp']}, def = {$user['def']}, hp = {$user['hp']} WHERE id = $user_id");
    $msg .= "<span style='color: #ffd700;'><b>🎉 恭喜升級至 Lv.{$user['level']}！全屬性提升！</b></span><br>";
}

// --- 計算訓練狀態 ---
$now = new DateTime();
$last_train = $user['last_train_time'] ? new DateTime($user['last_train_time']) : null;
$train_state = 'idle'; 
$remaining_cd = 0;
if ($last_train !== null) {
    $seconds_diff = $now->getTimestamp() - $last_train->getTimestamp();
    if ($seconds_diff >= $cooldown_seconds) {
        ...
        $remaining_cd = $cooldown_seconds - $seconds_diff;
    }
}
