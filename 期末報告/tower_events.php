...
if ($event === 'merchant') {
    ...
    if (($auto['mode'] ?? 'manual') === 'manual') {
        $form_html = "<p>「知識就是力量，給我 $cost 金幣，我傳授你 $gain 經驗值。」</p><form method='post' style='display:flex; gap:10px; margin-top:10px;' onsubmit='if(this.dataset.s)return false;this.dataset.s=1;this.querySelectorAll(\"button\").forEach(b=>{b.style.opacity=\"0.5\";b.style.pointerEvents=\"none\";});'>" . csrf_field() . "<button type='submit' name='action' value='exp_yes' class='btn-action' style='background:#4caf50;'>💰 支付金幣</button><button type='submit' name='action' value='exp_no' class='btn-action' style='background:#757575;'>🚶 轉身離開</button></form>";
        $add_line($form_html, 100);
        $stop_loop = true;
    } else {
